...
# Read in data
jumps <- as.data.frame(read_dta("./data_prepared.dta")) %>%
    select(order(names(.))) %>%
    arrange(jumpid)
tbl_df(jumps)
...
We propose several hypotheses on biases in international ski jumping competitions, based on the literature reviewed above. On the basis of earlier findings, we propose a first hypothesis that there are nationalistic biases in ski jumping. Specifically, we expect there to be a positive bias in scores when judges score performances by athletes who are from the same nation as themselves. 

Our second hypotheses states that the gross bias is partly explained by controlling for characteristics of the jump and athlete. The empirical implication of is that the magnitude of the bias will attenuate once all unobserved factors related to the jump are controlled. 

A third hypothesis is that this bias may be partly due to cultural influences on stylistic elements, and that the strength of the bias is lower in countries with strong traditions for ski jumping and higher in countries where the sport is relatively less important.


For home field advantages, we propose that home field advantages are measurable and impacts athletes performances. We further distinguish between the social aspects of competing on the home field and the physical aspects of competing on the home field. We hypothesize that the physical home field advantage is less salient than the social home field advantage, and thus less important for performances.

# DATA AND METHODS

To test the hypotheses proposed above, we use data from the FIS World Cup competitions in ski jumping from the 2006/07 and 2007/08 seasons. The original data were downloaded from the FIS web site, coded, and collated into complete data set for analysis. Our data include detailed information on each jump in each competition, including the five individual judge scores from the jump along with the identity and nationality of each judge in each competition. We also know where the competition was held.

Our data are now about a decade old, an age that in many social science areas may raise questions about the value of our analysis. We have made efforts to obtain newer and larger-scale data from owners of data on skijumping competitions. Our requests made to both Federation International de Ski and the Norwegian Skiing Assocation were rejected for legal reasons. However, even if more recent data would be slighly preferable, we are not worried about the analysis' "expiry date". The reason is that the phenomena under study, nationalist and home field biases, are neither new nor old, but general in nature and similar phenomena are likely to persist in any kind of evaluation setting. Our analysis, we would argue, has relevance far beyond the scope of our data, the skijumping sport, and perhaps even beyond the sporting domain.

We exclude team events from both seasons, leaving us with a data set of 51 competitions and 3,772 individual jumps. Table 1 outlines the dimensionality of the data set. The total score of the jump is the sum of the length score and the stylistic score. In our data set, individual judge evaluations of each jump are available, as well as additional technical data on the length and speed of each contestant’s jumps.

```{r tab1, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
# TABLE 1. Descriptive statistic
descheading <- c("Statistic","Mean / Number"); numdigs = 2
thetable <- as.data.frame(rbind( 
 c("No. of competitor nations", length(unique(jumps$competitornation))),
 c("No. of unique skijumping hills", length(unique(jumps$competitionplace))),
 c("No. of unique competitors",length(unique(jumps$competitorname))),
 c("No. of unique judges", length(unique(d$judge_name))),
 c("No. of jumps", nrow(jumps)),
 c("No. of competitions", length(unique(jumps$competition))),
 c("Avg. judge score", as.numeric(round(mean(as.numeric(d$judge)),numdigs))),
 c("Avg. jump length", as.numeric(round(mean(d$length),numdigs))),
 c("Avg. points", as.numeric(round(mean(d$points),numdigs))), deparse.level=0 ))
colnames(thetable) <- descheading
pander(thetable)
```

The analysis proceeds in several steps. First, we estimate nationalistic biases using a sequence of regression models where more complexity is incorporated for each model. Then, we estimate home field advantage. Finally, we simulate how important nationalistic biases and home field advantages are in final rankings of the competitions.

## Estimation of nationalistic biases

Following the general approach used in most studies of nationalistic biases (in particular, Emerson et al. (2009) and Zitzewitz (2006)), we analyse systematic tendencies in scores by individual judges and by the judges’ nationality. We begin with a simple model and add complexity in subsequent steps to approach a stricter test of nationalistic bias. In the first, simple, model, a dummy variable $\phi(I=J)$ indicates whether the judge comes from the same country (J) as the athlete (I) or not. The model can be written as
...
Our analysis of nationalistic bias proceeds in four steps. Table 2 presents the results from the first three estimations. The fluctuation in the number of observations is due to jumps with missing values on one or more of the independent variables, which have been deleted from the estimation.

\begin{landscape}
```{r tab2, echo=FALSE, results='asis'}
stargazer(model1, model2, model3, out.header=FALSE, header=FALSE,
    single.row=TRUE,  
    column.labels = c("Model 1: Naïve", "Model 2: Within-jump", "Model 3: Within-judge"),
    ...
    add.lines = c(" "),
    notes.align="l",
    notes = c("In models 2 and 3, the intercept is suppressed by the fixed effects estimation procedure."),
    notes.append=TRUE,
...
figure2 <- ggplot(data = nationbias,mapping = aes(x = ocountry, y = estimate)) +
    geom_point(size=3) +  
    geom_pointrange(aes(x=country, y=estimate, ymin = lowci, ymax=hici), fatten = 1.5) +
    #color="grey") +
...
We estimated nationalistic and home-field advantages in ski jumping using data from the entire FIS World Cup 2006/2007 and 2007/2008 seasons. From our results, we confirm parts of the extant literature, and document both national and home field biases in judges’ evaluations. The biases were generally small, and do not seem to pose a major threat to the validity of results from skijumping competitions. However, in league or series-type competition tournaments, the cumulative effects of natioanlistic bias may be important. Future research should emphasize this problem.

Biases in subjective evaluations of judges and referees may carry important implications for sports results, as exemplified by findings of potential effects on final rankings of athletes or on betting markets [@larsen_racial_2008, @price_racial_2010]. Given the historically political nature of sports—as exemplified in extreme cases by the boycotts of the United States and the Soviet Union of the Moscow and Los Angeles Olympics, respectively, the contested nature of the 1972 chess championships between Fisher and Spassky, and the many cases of violence between sports fans (and, at the most extreme end the alleged “Football War” between Honduras and El Salvador in 1969)—subjective biases in sports can even carry political implications. Knowledge of the sources of evaluative biases in competitive sports can help us better understand biases and discriminatory practices in other fields, such as student and employee recruitment and the allocation of research grants. Our findings of nationalistic and home-field biases have provided evidence that, in particular when assessed together with other related findings, add to our understanding on bias-generating practices. This knowledge may aid sport organizations, as well as other types of organizations, in their control of similar biases, and produce fairer, performance evaluations in the future.
